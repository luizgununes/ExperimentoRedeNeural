class Jogador {
  constructor() {
    ...
    this.nodes = [];
    this.fading = false;
    this.cerebro = new Cerebro(numberOfSteps);
    this.human = false;
    this.setNodes();
  }
  ...
  move() {
    if (!humanPlaying) {
      if (this.moveCount == 0) {//move in the direction for 6 frames
        if (this.cerebro.direcoes.length > this.cerebro.passo) {//if there are still direcoes left then set the velocity as the next PVector in the direcitons array
          this.vel = this.cerebro.direcoes[this.cerebro.passo];
          this.cerebro.passo++;
        } else {//if at the end of the direcoes array then the player is dead
          this.dead = true;
          this.fading = true;
        }
        this.moveCount = 6;
    ...
    for (var i = 0; i < solids.length; i++) {
      temp = solids[i].restrictMovement(this.pos, createVector(this.pos.x + this.tamanho, this.pos.y + this.tamanho), temp);
    }
    this.pos.add(temp);
  ...
  checkCollisions() {
    for (var i = 0; i < dots.length; i++) {
      if (dots[i].collides(this.pos, createVector(this.pos.x + this.tamanho, this.pos.y + this.tamanho))) {
        ...
        this.deathAtStep = this.cerebro.passo;
      }
    }
    if (winArea.collision(this.pos, createVector(this.pos.x + this.tamanho, this.pos.y + this.tamanho))) {
      this.reachedGoal = true;
    }
    for (var i = 0; i < this.nodes.length; i++) {
      this.nodes[i].collision(this.pos, createVector(this.pos.x + this.tamanho, this.pos.y + this.tamanho));
    }
  }
  ...
  calculateFitness() {
    if (this.reachedGoal) {//if the dot reached the goal then the fitness is based on the amount of steps it took to get there
      this.fitness = 1.0 / 16.0 + 10000.0 / (this.cerebro.passo * this.cerebro.passo);
  ...
  gimmeBaby() {
    var baby = new Jogador();
    baby.cerebro = this.cerebro.clone();//babies have the same cerebro as their parents
