class Populacao {
  ...
  //update all players
   update() {
    for (var i = 0; i< this.players.length; i++) {
      if (this.players[i].cerebro.passo > this.minStep) {//if the player has already taken more steps than the best player has taken to reach the goal
  ...
  //mutates all the cerebros of the babies
   mutateDemBabies() {
    for (var i = 1; i< this.players.length; i++) {
      this.players[i].cerebro.mutate(this.players[i].deathByDot, this.players[i].deathAtStep);
 ...
 increaseMoves() {
    if (this.players[0].cerebro.direcoes.length < 120 && !this.solutionFound) {
      for (var i = 0; i< this.players.length; i++) {
        this.players[i].cerebro.increaseMoves();
      }
    }
  }
}